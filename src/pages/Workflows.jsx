    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 }}>
        {workflows.map((wf) => {
          ...
          return (
            <div key={wf.id} className="card" style={{ overflow: 'hidden', transition: 'box-shadow 0.2s', borderColor: isRunning ? 'rgba(99,102,241,0.3)' : undefined }}
              onMouseEnter={(e) => e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.08)'}
              onMouseLeave={(e) => e.currentTarget.style.boxShadow = 'none'}
            >
              {/* Running banner */}
              {isRunning && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 20px', background: 'rgba(99,102,241,0.08)', borderBottom: '1px solid rgba(99,102,241,0.15)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      {isRunning && <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 999, background: 'rgba(99,102,241,0.12)', color: 'var(--brand)' }}>In Progress</span>}
                    </div>
                ...
                <div style={{ borderTop: '1px solid var(--border)', padding: '14px 20px', background: 'var(--surface-2)' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {wf.stepsDetail.map((step, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: isActiveStep ? '8px 10px' : '2px 0', borderRadius: isActiveStep ? 8 : 0, background: isActiveStep ? 'rgba(99,102,241,0.07)' : 'transparent', border: isActiveStep ? '1px solid rgba(99,102,241,0.2)' : '1px solid transparent', transition: 'all 0.15s' }}>
                          <div style={{ width: 22, height: 22, borderRadius: '50%', background: isDoneStep ? 'rgba(16,185,129,0.12)' : isActiveStep ? 'rgba(99,102,241,0.12)' : 'var(--surface)', border: `1px solid ${isDoneStep ? 'rgba(16,185,129,0.3)' : isActiveStep ? 'rgba(99,102,241,0.3)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: isDoneStep ? 'var(--success)' : isActiveStep ? 'var(--brand)' : 'var(--text-3)', fontWeight: 600, flexShrink: 0 }}>
                          ...
                          {isRunning ? (
                            <span style={{ fontSize: 10, fontWeight: 500, padding: '2px 7px', borderRadius: 999, background: isDoneStep ? 'rgba(16,185,129,0.1)' : isActiveStep ? 'rgba(99,102,241,0.1)' : 'rgba(148,163,184,0.1)', color: isDoneStep ? 'var(--success)' : isActiveStep ? 'var(--brand)' : 'var(--text-3)', flexShrink: 0 }}>
